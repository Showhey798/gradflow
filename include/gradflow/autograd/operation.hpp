 ...
 *
 * Operation represents a computation node in the automatic differentiation graph.
 * Each operation implements forward and backward passes for gradient computation.
 *
 * Design principles:
 * - Single Responsibility: Each operation class handles one specific computation
 * - Template Method Pattern: Base class defines the structure, derived classes implement specifics
 ...
 * 4. Use save_for_backward() to store intermediate values needed for backward pass
 *
 * Example:
 * @code
 * template <typename T>
...
protected:
     ...
     * (inputs, outputs, or computed values) that will be needed during the backward pass.
     *
     * Example:
     * @code
     * // In forward():
...
private:
     ...
     * to this operation. They are used to traverse the graph during backpropagation.
     */
