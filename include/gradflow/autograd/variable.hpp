 ...
 * It wraps a Tensor and tracks the computational graph needed for
 * backpropagation.
 *
 * Design principles:
...
class Variable {
 public:
   ...
   * @param requires_grad Whether to compute gradients for this variable
   */
  Variable(const Tensor<T>& data, std::shared_ptr<Operation<T>> grad_fn,
  ...
  [[nodiscard]] const Tensor<T>& grad() const {
    if (!hasGrad()) {
      throw std::runtime_error(
          "Gradient has not been computed. Call backward() first.");
    }
    return grad_;
  }
   ...
   *
   * @return Shared pointer to the gradient function (nullptr if this is a leaf
   * variable)
   */
  [[nodiscard]] const std::shared_ptr<Operation<T>>& gradFn() const {
    return grad_fn_;
  }
   ...
   *
   * This is typically called when this variable is created as the output of an
   * operation.
   *
   * @param grad_fn The operation that created this variable
   */
  void setGradFn(std::shared_ptr<Operation<T>> grad_fn) {
    grad_fn_ = std::move(grad_fn);
  }
   ...
   *
   * A leaf variable is one that was created directly by the user (not as a
   * result of an operation). Leaf variables have grad_fn_ == nullptr.
   *
   ...
   *
   * This method implements automatic differentiation by traversing the
   * computational graph in reverse topological order and applying the chain
   * rule.
   *
   * Algorithm:
   ...
   *
   * @param retain_graph If true, keep the computational graph for multiple
   * backward passes (default: false)
 ...
 private:
   ...
   *
   * This method recursively traverses the computational graph and computes
   * gradients.
   *
  ...
  void backwardImpl(const Tensor<T>& grad_output, bool retain_graph) {
    ...
    if (inputs.size() != input_grads.size()) {
      throw std::runtime_error(
          "Mismatch between number of inputs and computed gradients");
    }
    ...
    for (size_t i = 0; i < inputs.size(); ++i) {
      ...
      if (input_var != nullptr && input_var->requiresGrad()) {
        ...
        if (!input_var->hasGrad()) {
          // Create a new tensor for gradient (deep copy to avoid sharing
          // storage)
    ...
    // If not retaining graph, clear the operation's saved tensors to free
    // memory
    if (!retain_graph) {
   ...
   * For intermediate variables (created by operations), this points to the
   * operation.
   */
