 ...
 * @param output_index Which output element to use for gradient check (default:
 * first element)
...
bool checkNumericalGradient(Operation<T>& op,
  ...
  for (size_t input_idx = 0; input_idx < inputs.size(); ++input_idx) {
    ...
    for (size_t i = 0; i < input.size(); ++i) {
      ...
      if (output_index.empty()) {
        output_val_plus =
            output_plus[std::vector<size_t>(output_plus.ndim(), 0)];
        output_val_minus =
            output_minus[std::vector<size_t>(output_minus.ndim(), 0)];
      } else {
      ...
      T numerical_grad =
          (output_val_plus - output_val_minus) / (T(2) * epsilon);
      ...
      T max_val =
          std::max(std::abs(numerical_grad), std::abs(analytical_grad_val));
