...
class CrossEntropyLossOperation : public Operation<T> {
 public:
  ...
  Tensor<T> forward(const std::vector<Tensor<T>>& inputs) override {
    if (inputs.size() != 2) {
      throw std::invalid_argument(
          "CrossEntropyLossOperation requires exactly 2 inputs");
    }
    ...
    if (logits.shape() != target.shape()) {
      throw std::invalid_argument(
          "CrossEntropyLossOperation: logits and target must have the same "
          "shape");
    }

    if (logits.ndim() != 2) {
      throw std::invalid_argument(
          "CrossEntropyLossOperation: logits must be 2D [batch_size, "
          "num_classes]");
    }
  ...
  [[nodiscard]] std::string name() const override {
    return "CrossEntropyLossOperation";
  }
};
 ...
 public:
  ...
  Tensor<T> forward(const std::vector<Tensor<T>>& inputs) override {
    ...
    for (size_t i = 0; i < predicted.size(); ++i) {
      clamped_pred.data()[i] =
          std::max(eps_, std::min(T(1) - eps_, predicted.data()[i]));
    }
    ...
    for (size_t i = 0; i < predicted.size(); ++i) {
      ...
      loss_elements.data()[i] =
          -(target_val * std::log(pred_val) +
  ...
  std::vector<Tensor<T>> backward(const Tensor<T>& grad_output) override {
    ...
    // grad_predicted = (pred - target) / (pred * (1 - pred) + eps) * (1/N) *
    // grad_output This is more numerically stable than the original formulation
    ...
    for (size_t i = 0; i < predicted.size(); ++i) {
      ...
      T target_val = target.data()[i];
      grad_predicted.data()[i] = scale * (pred_val - target_val) /
                                 (pred_val * (T(1) - pred_val) + eps_);
    }
