...
class LogSoftmaxOperation : public Operation<T> {
 public:
   ...
   * @brief Constructor
   *
   * @param dim Dimension along which to apply log softmax (default: -1, last
   * dim)
   */
  ...
  Tensor<T> forward(const std::vector<Tensor<T>>& inputs) override {
    if (inputs.size() != 1) {
      throw std::invalid_argument(
          "LogSoftmaxOperation requires exactly 1 input");
    }
    ...
    auto sum_exp =
        sum(exp_shifted, static_cast<size_t>(actual_dim), /*keepdim=*/true);
  ...
  std::vector<Tensor<T>> backward(const Tensor<T>& grad_output) override {
    ...
    auto sum_grad =
        sum(grad_output, static_cast<size_t>(actual_dim), /*keepdim=*/true);
