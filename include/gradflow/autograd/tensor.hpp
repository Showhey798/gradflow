 ...
 *
 * Tensor<T> is the fundamental data structure for representing N-dimensional
 * arrays. It supports various operations like reshaping, transposing, slicing,
 * and more.
 *
 * Design:
...
class Tensor {
 public:
  ...
  [[nodiscard]] bool isContiguous() const {
    return stride_.isContiguous(shape_);
  }
   ...
   * @throws std::invalid_argument if tensor is not contiguous or sizes don't
   * match
   */
  Tensor<T> view(const Shape& new_shape) const {
    if (!isContiguous()) {
      throw std::invalid_argument(
          "Tensor must be contiguous for view operation");
    }
   ...
   * Should be called at the beginning of tests or experiments for
   * reproducibility.
   *
  ...
  static Tensor<T> randn(
      const Shape& shape, T mean = T(0), T stddev = T(1),
      const std::shared_ptr<DeviceAllocator>& allocator = nullptr) {
    Tensor<T> result(shape, allocator);
  ...
  /**
   * @brief Creates a tensor filled with random values from uniform distribution
   * [0, 1)
   *
   * @param shape Shape of the tensor
   * @param allocator Device allocator (defaults to CPU allocator)
   ...
   */
  static Tensor<T> eye(
      size_t n, const std::shared_ptr<DeviceAllocator>& allocator = nullptr) {
 ...
 private:
   ...
   */
  Tensor(std::shared_ptr<Storage<T>> storage, Shape shape, Stride stride,
         size_t offset)
  ...
  void copyRecursive(Tensor<T>& dest, std::vector<size_t>& indices,
                     size_t dim) const {
    if (dim == ndim()) {
