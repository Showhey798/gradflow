...
class Adam : public Optimizer<T> {
 public:
   ...
   */
  explicit Adam(T lr = T(0.001), T beta1 = T(0.9), T beta2 = T(0.999),
                T epsilon = T(1e-8), T weight_decay = T(0.0),
                bool adamw = false)
      : lr_(lr),
        beta1_(beta1),
        beta2_(beta2),
  ...
  void step() override {
    ...
    for (auto* param : this->params_) {
      ...
      for (size_t i = 0; i < kDataSize; ++i) {
        ...
        // Update biased first moment estimate: m_t = beta1 * m_{t-1} + (1 -
        // beta1) * g_t
        m_ptr[i] = beta1_ * m_ptr[i] + (T(1) - beta1_) * g;

        // Update biased second moment estimate: v_t = beta2 * v_{t-1} + (1 -
        // beta2) * g_t^2
        ...
        if (adamw_ && weight_decay_ > kZeroThreshold) {
          ...
          // Standard Adam (with or without L2 regularization already in
          // gradient)
 ...
 private:
  ...
  T beta1_power_;         ///< Cumulative product of beta1 (for efficient bias
                          ///< correction)
  T beta2_power_;         ///< Cumulative product of beta2 (for efficient bias
                          ///< correction)
