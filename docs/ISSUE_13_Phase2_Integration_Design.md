...
1. **十分な Epoch 数**: 5000 epoch 程度
2. **適切な学習率**: Adam で lr=0.01 は良い初期値
3. **隠れ層のサイズ**: 4 次元で十分
...
1. **シード固定**: テスト開始時に乱数シードを固定（オプション）
2. **複数回実行**: CI で複数回実行して安定性を確認
3. **緩い閾値**: 精度 90% は達成可能な現実的な値
