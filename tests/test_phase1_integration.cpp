...
class Phase1IntegrationTest : public ::testing::Test {
 protected:
  ...
  // Sanitizers add significant overhead, so we use smaller sizes to prevent
  // timeouts
  size_t getLargeTestSize(size_t normal_size) {
#if defined(__has_feature)
#if __has_feature(address_sanitizer) || __has_feature(thread_sanitizer) || \
    __has_feature(memory_sanitizer)
    return std::max(size_t{10},
                    normal_size / 50);  // 50分の1のサイズを使用（最小10）
#endif
#endif
...
#if __has_feature(address_sanitizer) || __has_feature(thread_sanitizer) || \
    __has_feature(memory_sanitizer)
    return std::max(size_t{5},
                    normal_count / 20);  // 20分の1の反復回数を使用（最小5）
#endif
#endif
#if defined(__SANITIZE_ADDRESS__) || defined(__SANITIZE_THREAD__)
    return std::max(size_t{5},
...
TEST_F(Phase1IntegrationTest, TransposeAndOperations) {
  ...
  auto ones_times_two =
      mul(Tensor<float>::ones({40, 50}), Tensor<float>({2.0F}));
...
TEST_F(Phase1IntegrationTest, LargeScaleOperations) {
  ...
  auto ones_times_half =
      mul(Tensor<float>::ones({rows, cols}), Tensor<float>({0.5F}));
