...
TEST_F(LossGradTest, MSELossNumericalGradient) {
  ...
  const float tolerance =
      2e-3F;  // Slightly relaxed tolerance for numerical precision
  ...
  for (size_t i = 0; i < predicted.size(); ++i) {
    ...
    float numerical_grad =
        (loss_plus.data()[0] - loss_minus.data()[0]) / (2.0F * epsilon);
...
TEST_F(LossGradTest, NLLLossForward) {
  ...
  auto log_probs =
      Tensor<float>({{-1.0F, -2.0F, -3.0F}, {-0.5F, -1.5F, -2.5F}});
...
TEST_F(LossGradTest, CrossEntropyLossBackward) {
  ...
  // (for each sample)
  float sum_grad_sample_0 =
      grads[0].data()[0] + grads[0].data()[1] + grads[0].data()[2];
  float sum_grad_sample_1 =
      grads[0].data()[3] + grads[0].data()[4] + grads[0].data()[5];
...
TEST_F(LossGradTest, CrossEntropyLossLargeLogits) {
  ...
  auto logits =
      Tensor<float>({{1000.0F, 999.0F, 998.0F}, {500.0F, 501.0F, 499.0F}});
  auto target = Tensor<float>({{1.0F, 0.0F, 0.0F}, {0.0F, 1.0F, 0.0F}});
...
TEST_F(LossGradTest, CrossEntropyLossConvergence) {
  ...
  auto target = Tensor<float>({{1.0F, 0.0F}, {0.0F, 1.0F}});
  auto logits =
      Tensor<float>({{0.0F, 0.0F}, {0.0F, 0.0F}});  // start from zeros
