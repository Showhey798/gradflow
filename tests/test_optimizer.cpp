...
class OptimizerTest : public ::testing::Test {
 protected:
  /**
   * @brief Helper function to optimize a quadratic function: f(x) = (x -
   * target)^2
   *
   ...
   */
  template <typename T>
  T optimize_quadratic(optim::Optimizer<T>* optimizer, Variable<T>& param,
                       T target, int num_steps) {
...
TEST_F(OptimizerTest, SGDConvergence) {
  ...
  // Optimize
  float final_value =
      optimize_quadratic(&optimizer, param, target_value, num_steps);
...
TEST_F(OptimizerTest, AdamWConvergence) {
  ...
  float learning_rate = 0.1f;
  auto optimizer = optim::Adam<float>(learning_rate, 0.9f, 0.999f, 1e-8f,
                                      weight_decay, true);
  optimizer.addParamGroup({&param});
...
TEST_F(OptimizerTest, InvalidHyperparameters) {
  ...
  // Invalid epsilon
  EXPECT_THROW(optim::Adam<float>(0.01f, 0.9f, 0.999f, -1e-8f),
               std::invalid_argument);

  // Invalid weight decay
  EXPECT_THROW(optim::SGD<float>(0.01f, 0.0f, -0.01f), std::invalid_argument);
  EXPECT_THROW(optim::Adam<float>(0.01f, 0.9f, 0.999f, 1e-8f, -0.01f),
               std::invalid_argument);
}
